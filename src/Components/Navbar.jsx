...
function Navbar() {
  ...
  return (
    <div className={`w-full h-[10%] fixed p-2 flex items-center bg-amber-950 font-serif`}>
        ...
        {/* mobile navigation */}
        <div className={`absolute md:hidden ${isOpen ? 'left-0': 'left-[-100%]' } transition-all duration-400 h-[100vh] bottom-0 inset-0 w-[80%] bg-amber-950 z-10`}>
            <ul className={`mt-20 ml-20`}>
                {
                    navigationLinks.map((item) => (
                        <li key={item.id} className={`text-2xl text-white mb-2`}><a href={item.link}>{item.title}</a></li>
                    ))
                }
            </ul>
